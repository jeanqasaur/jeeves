...
# from templatetags.concretizetag import request_wrapper
import forms
    ...
    def real_view_fn(request):
        try:
            ...
            for key, value in context_dict.iteritems() :
                print key, concretize(value)
                context_dict[key] = concretize(value)

            #print 'concretized is', concretize(context_dict['latest_title'])

            # CHANGED THIS TO SEND THE WHOLE CONTEXT_DICT
            # r = render(request, template_name, {'context_dict':context_dict})
            r = render(request, template_name, context_dict)
