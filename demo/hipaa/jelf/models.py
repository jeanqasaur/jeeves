...
class Individual(Model):
	...
	def jeeves_get_private_Address(individual):
		...
		newAddress=Address.objects.create(City=individual.Address.City,State=individual.Address.State,ZipCode=zipCode)
		return newAddress
...
class CoveredEntity(Model):
	...
	"""
	EIN = CharField(max_length=9, blank=False, null=False, help_text="Government issued Employer Identification Number, should be uniquely identifiable")
...
class Treatment(Model):
	...
	PrescribingEntity = ForeignKey(CoveredEntity, null=True, related_name="Prescriptions", help_text="Entity that prescribed this service")
	PerformingEntity = ForeignKey(CoveredEntity, null=True, help_text="Entity that performed this service")
